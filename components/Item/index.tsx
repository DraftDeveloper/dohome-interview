...
const Items = ({ id, title, color, image, checkList }: ItemsType) => {
  const [checkListValue, setCheckListValue] =
    useState<Array<CheckList>>(checkList);
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "overflow-hidden rounded-lg bg-gradient-to-b shadow-sm",
          bg_gradient
        )}
      >
        ...
        <div className="px-4 py-5 sm:p-6">
          {image && (
            <div
              className={clsx(
                "relative w-full border-4 rounded-2xl",
                border_300
              )}
            >
              <img
                ...
                src={image}
                className={clsx(
                  "aspect-video w-full rounded-2xl object-cover sm:aspect-2/1 lg:aspect-3/2",
                  bg_100
                )}
              />
            </div>
          ...
          <span
            className={clsx(
              "mt-1 inline-block break-words w-full overflow-hidden",
              text_500
            )}
          >
            {title}
                  ...
                  <div className="flex h-6 shrink-0 items-center">
                    <div className="group grid size-4 grid-cols-1">
                      <input
                        ...
                        className={clsx(
                          "col-start-1 row-start-1 appearance-none rounded-sm border bg-white forced-colors:appearance-auto focus-visible:outline-2 focus-visible:outline-offset-2",
                          border_300,
                          checked_bg_600,
                          checked_border_600,
                          indeterminate_border_600,
                          indeterminate_bg_600,
                          focus_visible_outline_600,
                          disabled_border_300,
                          disabled_bg_100,
                          disabled_checked_bg_100
                        )}
                      />
                      <svg
                        fill="none"
          ...
          {checkListValue && (
            <>
              <p
                className={clsx(
                  "text-sm font-medium mt-4 text-[8px] leading-[8px]",
                  text_900
                )}
              >
                Progress : {status}
              ...
              <div aria-hidden="true" className="mt-1">
                <div className={clsx("overflow-hidden rounded-full", bg_200)}>
                  <div
                    style={{ width: status }}
                    className={clsx("h-2 rounded-full", bg_600)}
                  />
                </div>
              </div>
            </>
